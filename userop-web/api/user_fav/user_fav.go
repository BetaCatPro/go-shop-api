...
import (
	"context"
	"go-shop-api/userop-web/api"
	"go-shop-api/userop-web/forms"
	"go-shop-api/userop-web/global"
	"go-shop-api/userop-web/proto"
	"net/http"
	"strconv"
...
func New(ctx *gin.Context) {
	...
	if _, err := global.GoodsSrvClient.GetGoodsDetail(context.Background(), &proto.GoodInfoRequest{Id: userFavForm.GoodsId}); err != nil {
		zap.S().Errorw("该商品不存在")
		api.HandleGrpcErrorToHttp(err, ctx)
		return
	}
