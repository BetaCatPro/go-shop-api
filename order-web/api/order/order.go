...
import (
	"context"
	"go-shop-api/order-web/forms"
	"go-shop-api/order-web/global"
	"go-shop-api/order-web/proto"
	"net/http"
	"strconv"
	...
	"go-shop-api/order-web/api"
	"go-shop-api/order-web/models"
)
...
func New(ctx *gin.Context) {
	...
	userId, _ := ctx.Get("userId")
	rsp, err := global.OrderSrvClient.CreateOrder(context.WithValue(context.Background(), myString("ginContext"), ctx), &proto.OrderRequest{
	...
	p.Subject = "go-shop订单-" + rsp.OrderSn
...
func Detail(ctx *gin.Context) {
	...
	p.NotifyURL = global.ServerConfig.AliPayInfo.NotifyURL
	p.ReturnURL = global.ServerConfig.AliPayInfo.ReturnURL
	p.Subject = "go-shop订单-" + rsp.OrderInfo.OrderSn
